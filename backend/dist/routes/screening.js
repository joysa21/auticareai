...
router.post("/results/video", upload.single("video"), async (req, res) => {
...
router.get("/results/:childId/latest-video", async (req, res) => {
    ...
    try {
        ...
        const { data: signedData, error: signedError } = await supabase_1.supabase.storage
            .from(SCREENING_VIDEO_BUCKET)
