...
export default function Screening() {
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      const reportPayload = result?.screening_report ?? result;
      const confidencePayload = result?.confidence_score ?? null;

      clearInterval(interval);

      setScreeningResult(reportPayload);

      // Map backend risk → UI risk
      const level = reportPayload.risk_assessment?.level?.toLowerCase?.() || "medium";
      ...
      const indicators = Object.entries(reportPayload.metrics?.behavioral_indicators || {})
      ...
      const { error: saveError } = await screeningService.saveResult({
        ...
        cvReport: {
          ...reportPayload,
          confidence_score: confidencePayload,
        },
      });
      if (saveError) {
        ...
        {step === "results" && (
          ...
          >
            ...
            {screeningStatus === "pending" && (
              <div className="mb-6 rounded-2xl border border-teal-200 bg-teal-50/50 p-6">
                <div className="flex items-center gap-3">
                  <AlertCircle className="h-6 w-6 text-teal-600" />
                  <div>
                    <p className="font-semibold text-teal-700">Pending Review</p>
                    <p className="text-sm text-teal-600">Awaiting doctor's clinical review</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="rounded-2xl bg-amber-50 border border-amber-200 p-8">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <span className="text-sm text-amber-700">Generated by Screening Agent</span>
                  </div>

                  <h2 className="text-2xl font-bold mb-2">
                    Risk Assessment:{" "}
                    <span className={riskConfig[riskLevel].color}>
                      {screeningResult?.risk_assessment?.level || `${riskLevel.charAt(0).toUpperCase() + riskLevel.slice(1)} Risk`}
                    </span>
                  </h2>
                  <p className="text-amber-800 mb-6">
                    {screeningResult?.risk_assessment?.description || riskConfig[riskLevel].message}
                  </p>

                  {/* Objective Signals */}
                  <div className="mb-6">
                    <h3 className="font-semibold text-amber-900 mb-4 flex items-center gap-2">
                      <span className="text-lg">📊</span> Objective Signals
                    </h3>
                    {objectiveSignals.length > 0 ? (
                      <div className="grid gap-3 sm:grid-cols-2">
                        {objectiveSignals.map((signal, index) => (
                          <div key={index} className="rounded-lg bg-white p-4 border border-amber-100">
                            <p className="font-semibold text-amber-900 mb-1">{signal.label}</p>
                            <p className="text-lg font-bold text-amber-700">{signal.value}</p>
                            <p className="text-xs text-amber-600 mt-1">
                              ...
                              >
                                {signal.status === "above" ? "above baseline" : "below baseline"}
                              </span>
                            </p>
                          </div>
                        ))}
                      ...
                      <p className="text-amber-700">No objective signals available.</p>
                    )}
                  </div>
                  ...
                  {behavioralIndicators.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-amber-900 mb-3">Behavioral Indicators</h3>
                      <ul className="space-y-2">
                        {behavioralIndicators.map((indicator, index) => (
                          <li key={index} className="flex items-center gap-2 text-amber-800 capitalize">
                ...
                {/* 3-frame confidence intentionally hidden from parent portal */}
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-border bg-card p-6">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase">Current Status</p>
                    <p className="text-lg font-semibold text-teal-600 mt-1">
                      {screeningStatus === "pending" ? "Pending" : "Reviewed"}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {screeningStatus === "pending" ? "Waiting for doctor review" : "Doctor has reviewed"}
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-3">
                      <div className="bg-teal-600 text-white rounded-full w-8 h-8 flex items-center justify-center flex-shrink-0">
                        <Bot className="h-4 w-4" />
                      </div>
                      <div>
                        <p className="font-semibold text-sm">Screening Agent</p>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm">Clinical Summary Agent</p>
                ...
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6">
                  ...
                  <p className="text-xs text-blue-800">
                    AI supports decisions, humans remain in control. This screening is not a diagnosis. A qualified healthcare professional will review these results.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-2xl border border-teal-200 bg-teal-50 p-6">
              ...
              <p className="text-teal-800 mb-4 text-sm">
                Your screening has been saved and is pending doctor review. Your assigned healthcare provider can access these results from their portal.
              </p>
              <div className="flex gap-3 flex-wrap">
                <Button 
                  onClick={() => navigate(`/parent/progress?childId=${selectedChildId}`)}
